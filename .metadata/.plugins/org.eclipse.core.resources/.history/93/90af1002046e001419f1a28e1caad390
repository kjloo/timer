...
public class SecondsTextField extends JTextField{
	...
	public void validate() {
		if(!getText().isEmpty() && getText().matches("\\d+|-\\d+")) {
			if(Integer.valueOf(getText()) > MAX_SECONDS || Integer.valueOf(getText()) < MIN_SECONDS) 
	...
	public boolean valid() {
		if(getText().isEmpty() || !getText().matches("\\d+|-\\d+"))
			return false;
		return !(Integer.valueOf(getText()) > MAX_SECONDS || Integer.valueOf(getText()) < MIN_SECONDS);
	}
