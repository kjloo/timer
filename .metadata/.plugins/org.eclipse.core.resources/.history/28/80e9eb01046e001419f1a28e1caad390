...
public class MinutesTextField extends JTextField{
	...
	public void validate() {
		if(!getText().isEmpty() && getText().matches("\\d+|-\\d+")) {
			if(Integer.valueOf(getText()) > MAX_MINUTES || Integer.valueOf(getText()) < MIN_MINUTES) 
	...
	public boolean valid() {
		if(getText().isEmpty() || !getText().matches("\\d+|-\\d+"))
			return false;
		return !(Integer.valueOf(getText()) > MAX_MINUTES || Integer.valueOf(getText()) < MIN_MINUTES);
	}
